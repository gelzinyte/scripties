...
def prep_axis(ref_vals, pred_vals, figsize=(6, 9)):
    ...
    parity_label = f"RMSE: {rmse:.2f}; MAE: {mae:.2f}; \nR: {pearr:.2f}; error: {mpe:.1f}%"
...
ats = read("test.mace.xyz", ":")

dft_label = "efg"
mace_label = "mace_efg"

dft_efgs = extract_efgs(ats, efg_label=dft_label)
mace_efgs = extract_efgs(ats, efg_label=mace_label)

element = "Li"

dft_data = get_props(dft_efgs[element], element)
mace_data = get_props(mace_efgs[element], element)

outdir = Path(f"/raven/u/egg/mounted/EFGs/1.correlation/")
outdir.mkdir(exist_ok=True, parents=True)

for prop in mace_data.keys():

    if prop == "quaternions":
        continue

    dft_vals = dft_data[prop]
    mace_vals = mace_data[prop]

    ax_hist, ax_par = prep_axis(dft_vals, mace_vals, figsize=(4, 6))

    ax_par.set_xlabel(f"DFT {prop}")
    ax_par.set_ylabel(f"MACE {prop}")
    ax_hist.set_ylabel("Counts")

    ax_hist.set_title(f"{element}_{prop}")
    plt.savefig(outdir / f"{element}_{prop}.png", dpi=300, bbox_inches="tight")
